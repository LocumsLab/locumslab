...
<!-- MAIN APP -->
<div id="mainApp">
  <!-- Try Free banner - always visible when not logged in -->
  <div id="signup-banner" style="background: rgba(251, 191, 36, 0.1); border-bottom: 1px solid rgba(251, 191, 36, 0.3); padding: 10px 24px; text-align: center; display:none;">
    <span style="color: #fbbf24; font-size: 0.875rem;"> You're in Try Free mode  calculators are fully unlocked. <a href="#" onclick="showAuthScreen(); return false;" style="color: #10b981; font-weight: 600; text-decoration: underline;">Sign up free</a> to save scenarios and compare contracts.</span>
  </div>
  ...
  <header>
    ...
    <div class="header-right">
      ...
      <button class="header-btn secondary" onclick="showWizard()" id="wizardBtn" style="display:none;">Should I Go Locums?</button>
      <button class="header-btn" onclick="openGuideDrawer()" style="color: var(--accent); border-color: rgba(16,185,129,0.4);"> Guide</button>
      <button class="header-btn" onclick="showDashboard()">Dashboard</button>
      <button class="header-btn hidden" id="logoutBtn" onclick="handleLogout()">Logout</button>
    </div>
  </header>
  ...
  <div class="wizard" id="wizard">
    ...
    <div class="wizard-choices">
      <div class="choice-card" onclick="selectGoal('quickstart')" style="border-color: var(--accent); background: rgba(16,185,129,0.05);">
        <div class="choice-emoji"></div>
        <div class="choice-title" style="color: var(--accent);">Quick Start  New to Locums?</div>
        <div class="choice-desc">Step-by-step guided comparison from W-2 to locums. Start here if you're just exploring.</div>
      </div>
      ...
      <div class="choice-card" onclick="selectGoal('contract-eval')">
        <div class="choice-emoji"></div>
        <div class="choice-title">True Hourly Rate</div>
        <div class="choice-desc">See what a contract offer really pays after taxes, insurance, malpractice, and travel</div>
      </div>
      ...
      <div class="choice-card" onclick="selectGoal('scorp')">
        <div class="choice-emoji"></div>
        <div class="choice-title">S-Corp Evaluator</div>
        <div class="choice-desc">Should you form an S-Corporation? Compare tax implications and savings</div>
      ...
      <div class="choice-card" onclick="selectGoal('benchmarks')">
        <div class="choice-emoji"></div>
        <div class="choice-title">Market Benchmarks</div>
        <div class="choice-desc">Salary, malpractice, retirement & tax data by profession  sourced from BLS 2024</div>
        <div style="display:inline-block; margin-top:8px; background: var(--accent); color: white; font-size: 0.625rem; padding: 2px 8px; border-radius: 4px; font-weight: 700; letter-spacing: 0.05em;">PRO</div>
      </div>
      <div class="choice-card" onclick="selectGoal('explore')">
        <div class="choice-emoji">ç</div>
  ...
  <div class="dashboard hidden" id="dashboardView">
    <div class="dashboard-header">
      ...
      <div class="dashboard-actions">
        <button class="dash-btn" onclick="showComparisonView()"> Compare Scenarios</button>
        <button class="dash-btn" onclick="startNew()">+ New Scenario</button>
        <button class="dash-btn secondary" onclick="loadScenarios()"> Refresh</button>
      </div>
    </div>
  ...
  <div class="calculator hidden" id="calculatorView">
    ...
    <nav class="nav">
      <button class="nav-btn" onclick="switchPanel('quickstart', this)" style="background: rgba(16,185,129,0.15); border-color: var(--accent); color: var(--accent);"> Quick Start</button>
      ...
      <button class="nav-btn" onclick="switchPanel('true-hourly', this)">True Hourly Rate</button>
      <button class="nav-btn" onclick="switchPanel('scorp', this)">S-Corp <span style="background: var(--accent); color: white; font-size: 0.625rem; padding: 2px 6px; border-radius: 4px; margin-left: 4px; font-weight: 700;">PRO</span></button>
      ...
      <button class="nav-btn" onclick="switchPanel('benchmarks', this)">Benchmarks <span style="background: var(--accent); color: white; font-size: 0.625rem; padding: 2px 6px; border-radius: 4px; margin-left: 4px; font-weight: 700;">PRO</span></button>
    </nav>
    ...
    <main>
      ...
      <div class="panel" id="panel-quickstart">
        <div class="panel-header">
          <h2> Quick Start: W-2 to Locums</h2>
          <p>New to locums? Answer a few questions and we'll show you exactly what your financial picture looks like.</p>
        </div>

        <!-- Step indicators -->
        <div id="qs-steps" style="display:flex; gap:8px; margin-bottom:28px; align-items:center;">
          <div id="qs-step-1" style="display:flex; align-items:center; gap:6px;">
            <div style="width:28px; height:28px; border-radius:50%; background:var(--accent); color:white; display:flex; align-items:center; justify-content:center; font-weight:700; font-size:0.875rem;">1</div>
            <span style="font-size:0.875rem; font-weight:600; color:var(--accent);">Your W-2</span>
          </div>
          <div style="flex:1; height:2px; background:var(--border);"></div>
          <div id="qs-step-2" style="display:flex; align-items:center; gap:6px;">
            <div id="qs-dot-2" style="width:28px; height:28px; border-radius:50%; background:var(--border); color:var(--muted); display:flex; align-items:center; justify-content:center; font-weight:700; font-size:0.875rem;">2</div>
            <span id="qs-label-2" style="font-size:0.875rem; font-weight:600; color:var(--muted);">Locums Target</span>
          </div>
          <div style="flex:1; height:2px; background:var(--border);"></div>
          <div id="qs-step-3" style="display:flex; align-items:center; gap:6px;">
            <div id="qs-dot-3" style="width:28px; height:28px; border-radius:50%; background:var(--border); color:var(--muted); display:flex; align-items:center; justify-content:center; font-weight:700; font-size:0.875rem;">3</div>
            <span id="qs-label-3" style="font-size:0.875rem; font-weight:600; color:var(--muted);">Your Results</span>
          </div>
        </div>

        <!-- Step 1: W-2 Details -->
        <div id="qs-page-1" class="card">
          <div class="card-title">STEP 1  YOUR CURRENT W-2 POSITION</div>
          <div class="field">
            <label>Your Profession</label>
            <select id="qs-profession">
              <option value="crna">CRNA</option>
              <option value="md_hospitalist">Physician - Hospitalist</option>
              <option value="md_em">Physician - Emergency Medicine</option>
              <option value="md_anesthesia">Physician - Anesthesiology</option>
              <option value="md_fp">Physician - Family Practice</option>
          ...
          <div class="field">
            ...
            <input type="number" id="qs-salary" value="140000" min="0" step="1000">
          </div>
          <div class="field">
            <label>Employer Benefits Value (health, dental, vision, retirement match)</label>
            <input type="number" id="qs-benefits" value="28000" min="0" step="1000">
            <div style="font-size:0.8125rem; color:var(--muted); margin-top:4px;">Typical range: $20,000$35,000/year</div>
          </div>
          <div class="field">
            <label>Paid Time Off (days/year)</label>
            <input type="number" id="qs-pto" value="20" min="0" max="60">
          </div>
          <div class="field">
            <label>Annual Bonus ($)</label>
            <input type="number" id="qs-bonus" value="0" min="0" step="1000">
          </div>
          <button class="calc-btn" onclick="qsNext(1)" style="margin-top:8px;">Next: Locums Target </button>
        </div>

        <!-- Step 2: Locums Target -->
        <div id="qs-page-2" class="card hidden">
          <div class="card-title">STEP 2  YOUR LOCUMS TARGET</div>
          <div class="field">
            <label>Expected Hourly Rate ($)</label>
            <input type="number" id="qs-hourly" value="130" min="0">
            <div style="font-size:0.8125rem; color:var(--muted); margin-top:4px;" id="qs-rate-hint">CRNA locums avg: $120$160/hr</div>
          </div>
          <div class="field">
            <label>Hours per Week</label>
            <input type="number" id="qs-hours" value="40" min="0" max="80">
          </div>
          <div class="field">
            <label>Weeks Working per Year</label>
            <input type="number" id="qs-weeks" value="46" min="1" max="52">
            <div style="font-size:0.8125rem; color:var(--muted); margin-top:4px;">Most locums providers work 4450 weeks/year</div>
          </div>
          <div class="field">
            <label>Housing/Travel Stipend (weekly, if provided)</label>
            <input type="number" id="qs-stipend" value="1800" min="0" step="100">
          </div>
          <div class="field">
            <label>Will you need to buy your own health insurance?</label>
            <select id="qs-insurance-needed">
              <option value="yes">Yes  I'll need to buy my own</option>
              <option value="no">No  covered another way (spouse, etc.)</option>
            </select>
          </div>
          <div class="field" id="qs-insurance-field">
            <label>Estimated Annual Health Insurance Premium ($)</label>
            <input type="number" id="qs-insurance" value="18000" min="0" step="500">
            <div style="font-size:0.8125rem; color:var(--muted); margin-top:4px;">Individual market avg: $15,000$22,000/year</div>
          </div>
          <div style="display:flex; gap:12px; margin-top:8px;">
            <button class="calc-btn" onclick="qsBack(2)" style="background:var(--surface2); color:var(--text);"> Back</button>
            <button class="calc-btn" onclick="qsNext(2)">See My Results </button>
          </div>
        </div>

        <!-- Step 3: Results -->
        <div id="qs-page-3" class="hidden">
          <div class="two-col" style="margin-bottom:24px;">
            <div class="compare-card" id="qs-w2-card">
              <h4>Your W-2 Position</h4>
              <div class="row"><span class="row-label">Base Salary</span><span class="row-val" id="qs-r-salary"></span></div>
              <div class="row"><span class="row-label">Benefits Value</span><span class="row-val" id="qs-r-benefits"></span></div>
              <div class="row"><span class="row-label">Bonus</span><span class="row-val" id="qs-r-bonus"></span></div>
              <div class="row"><span class="row-label">Total Compensation</span><span class="row-val" id="qs-r-totalcomp"></span></div>
              <div class="row"><span class="row-label">Federal + State Tax (est.)</span><span class="row-val" id="qs-r-w2tax"></span></div>
              <div class="row" style="border-top:2px solid var(--accent); margin-top:8px; padding-top:8px;"><span class="row-label" style="font-weight:700;">Net Take-Home</span><span class="row-val" id="qs-r-w2net"></span></div>
            </div>
            <div class="compare-card" id="qs-locums-card">
              <h4>Locums (1099)</h4>
              <div class="row"><span class="row-label">Gross Income</span><span class="row-val" id="qs-r-gross"></span></div>
              <div class="row"><span class="row-label">Housing Stipend (tax-free)</span><span class="row-val" id="qs-r-stipend"></span></div>
              <div class="row"><span class="row-label">Health Insurance</span><span class="row-val" id="qs-r-insurance"></span></div>
              <div class="row"><span class="row-label">Self-Employment Tax</span><span class="row-val" id="qs-r-setax"></span></div>
              <div class="row"><span class="row-label">Federal Income Tax (est.)</span><span class="row-val" id="qs-r-fedtax"></span></div>
              <div class="row" style="border-top:2px solid var(--accent); margin-top:8px; padding-top:8px;"><span class="row-label" style="font-weight:700;">Net Take-Home</span><span class="row-val" id="qs-r-locumsnet"></span></div>
            </div>
          </div>

          <div class="insight" style="margin-bottom:24px;">
            <div class="insight-label">QUICK START SUMMARY</div>
            <p id="qs-insight"></p>
          </div>

          <div style="background:var(--surface); border:1px solid var(--border); border-radius:12px; padding:20px; margin-bottom:24px;">
            <div style="font-weight:700; color:var(--text); margin-bottom:12px;"> Your Next Steps</div>
            <div id="qs-next-steps" style="font-size:0.9375rem; color:var(--muted); line-height:1.8;"></div>
          </div>

          <div style="display:flex; gap:12px; flex-wrap:wrap;">
            <button class="calc-btn" onclick="qsBack(3)" style="background:var(--surface2); color:var(--text);"> Edit Inputs</button>
            <button class="calc-btn" onclick="shareResults('quickstart')"> Share Results</button>
            <button class="calc-btn" onclick="switchPanel('scorp', document.querySelector('.nav-btn'))" style="background: rgba(16,185,129,0.15); color:var(--accent); border:1px solid var(--accent);">Check S-Corp Savings </button>
          </div>
        </div>
      </div>
      ...
      <div class="panel active" id="panel-staff-locums">
        ...
        <!-- Profession Selector for Benchmarks -->
        <div class="card" style="margin-bottom: 24px;">
          <div class="field">
            <label>Your Profession (for market benchmarks)</label>
            <select id="sl-profession">
              <option value="crna">CRNA</option>
              <option value="md_hospitalist">Physician - Hospitalist</option>
              <option value="md_em">Physician - Emergency Medicine</option>
              <option value="md_anesthesia">Physician - Anesthesiology</option>
        ...
        <div class="two-col">
          ...
          <div class="card">
            ...
            <button class="calc-btn" onclick="calcStaffLocums()">Compare </button>
          </div>
        </div>
        <div id="sl-results" class="hidden">
          <div class="two-col">
            <div class="compare-card" id="staff-card">
              ...
              <div class="row">
                ...
                <span class="row-val" id="staff-base"></span>
              </div>
              <div class="row">
                <span class="row-label">Benefits Value</span>
                <span class="row-val" id="staff-ben"></span>
              </div>
              <div class="row">
                <span class="row-label">Total Compensation</span>
                <span class="row-val" id="staff-total"></span>
              </div>
              <div class="row">
                <span class="row-label">Taxes (Est. 25%)</span>
                <span class="row-val" id="staff-tax"></span>
              </div>
              <div class="row">
                <span class="row-label">Net Take-Home</span>
                <span class="row-val" id="staff-net"></span>
              </div>
            </div>
            <div class="compare-card" id="locums-card">
              ...
              <div class="row">
                ...
                <span class="row-val" id="locums-gross"></span>
              </div>
              <div class="row">
                <span class="row-label">Total Expenses</span>
                <span class="row-val" id="locums-exp"></span>
              </div>
              <div class="row">
                <span class="row-label">Taxable Income</span>
                <span class="row-val" id="locums-taxable"></span>
              </div>
              <div class="row">
                <span class="row-label">Taxes (SE + Fed)</span>
                <span class="row-val" id="locums-tax"></span>
              </div>
              <div class="row">
                <span class="row-label">Net Take-Home</span>
                <span class="row-val" id="locums-net"></span>
              </div>
              <div class="row">
                <span class="row-label">Difference vs Staff</span>
                <span class="row-val" id="locums-diff"></span>
              </div>
            </div>
          </div>
          ...
          <!-- MARKET BENCHMARKS -->
          <div id="sl-benchmarks" class="hidden" style="margin-top: 32px; padding: 24px; background: linear-gradient(135deg, rgba(16, 185, 129, 0.05), rgba(59, 130, 246, 0.05)); border: 1px solid var(--border); border-radius: 12px;">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
              <h4 style="font-size: 1.125rem; font-weight: 700; color: var(--text); margin: 0;">Market Benchmarks</h4>
              <span style="font-size: 0.75rem; color: var(--muted); background: white; padding: 4px 10px; border-radius: 6px; border: 1px solid var(--border);">Source: BLS 2023, MGMA 2023</span>
            </div>
            
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
              <!-- Staff Salary Benchmark -->
              <div style="background: white; border: 1px solid var(--border); border-radius: 8px; padding: 16px;">
                <div style="font-size: 0.875rem; font-weight: 600; color: var(--muted); margin-bottom: 8px;">STAFF SALARY</div>
                <div style="font-size: 1.5rem; font-weight: 700; color: var(--text); margin-bottom: 4px;" id="benchmark-staff-salary"></div>
                <div id="benchmark-staff-badge" style="display: inline-block; padding: 4px 12px; border-radius: 6px; font-size: 0.8125rem; font-weight: 600; margin-bottom: 8px;"></div>
                <div style="font-size: 0.8125rem; color: var(--muted);" id="benchmark-staff-detail"></div>
              </div>
              
              <!-- Locums Rate Benchmark -->
              <div style="background: white; border: 1px solid var(--border); border-radius: 8px; padding: 16px;">
                <div style="font-size: 0.875rem; font-weight: 600; color: var(--muted); margin-bottom: 8px;">LOCUMS RATE</div>
                <div style="font-size: 1.5rem; font-weight: 700; color: var(--text); margin-bottom: 4px;" id="benchmark-locums-rate"></div>
                <div id="benchmark-locums-badge" style="display: inline-block; padding: 4px 12px; border-radius: 6px; font-size: 0.8125rem; font-weight: 600; margin-bottom: 8px;"></div>
                <div style="font-size: 0.8125rem; color: var(--muted);" id="benchmark-locums-detail"></div>
              </div>
            </div>
            
            <div style="margin-top: 16px; padding: 12px; background: rgba(59, 130, 246, 0.1); border-radius: 6px; font-size: 0.875rem; color: var(--text);">
               <strong>Tip:</strong> <span id="benchmark-tip">Benchmark data helps you evaluate if your offer is competitive.</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- TRUE HOURLY RATE -->
      <div class="panel" id="panel-true-hourly">
        <div class="panel-header">
          <h2>True Hourly Rate Calculator</h2>
          <p>Your contract rate isn't your real rate. See what you actually earn after taxes, benefits costs, and time.</p>
        </div>
        <div class="two-col">
          <div class="card">
            ...
            <div class="field">
              ...
              <select id="thr-type" onchange="thrToggleType()">
                <option value="hourly">Hourly Rate</option>
                <option value="weekly">Weekly Rate</option>
                <option value="annual">Annual / 1099 Income</option>
              </select>
            </div>
            <div class="field" id="thr-hourly-field">
              <label>Hourly Rate ($)</label>
              <input type="number" id="thr-hourly" value="130" min="0">
            </div>
            <div class="field" id="thr-weekly-field" style="display:none;">
              <label>Weekly Rate ($)</label>
              <input type="number" id="thr-weekly" value="5200" min="0" step="100">
            </div>
            <div class="field" id="thr-annual-field" style="display:none;">
              <label>Annual Income ($)</label>
              <input type="number" id="thr-annual" value="250000" min="0" step="1000">
            </div>
            <div class="field">
              <label>Hours Worked per Week (clinical)</label>
              <input type="number" id="thr-clinical-hours" value="40" min="1" max="80">
            </div>
            <div class="field">
              <label>Weeks Worked per Year</label>
              <input type="number" id="thr-weeks" value="46" min="1" max="52">
            </div>
          </div>
          <div class="card">
            <div class="card-title">REAL COSTS TO SUBTRACT</div>
            <div class="field">
              <label>Travel Time per Week (hours, round trip)</label>
              <input type="number" id="thr-travel-time" value="4" min="0" max="40">
              <div style="font-size:0.8125rem; color:var(--muted); margin-top:4px;">Time traveling is unpaid  it reduces your true rate</div>
            </div>
            <div class="field">
              <label>Annual Travel Expenses (flights, gas, lodging if not covered)</label>
              <input type="number" id="thr-travel-cost" value="6000" min="0" step="500">
            </div>
            <div class="field">
              <label>Annual Health Insurance Premium ($)</label>
              <input type="number" id="thr-health" value="18000" min="0" step="500">
              <div style="font-size:0.8125rem; color:var(--muted); margin-top:4px;">Individual market avg: $15,000$22,000/year</div>
            </div>
            <div class="field">
              <label>Annual Malpractice Insurance Premium ($)</label>
              <input type="number" id="thr-malpractice" value="3000" min="0" step="500">
              <div style="font-size:0.8125rem; color:var(--muted); margin-top:4px;">National avg: CRNAs ~$2,500$4,000 | Physicians ~$5,000$15,000+</div>
            </div>
            <div class="field">
              <label>Annual Benefits Gap (dental, vision, disability, life)</label>
              <input type="number" id="thr-benefits-gap" value="4000" min="0" step="500">
              <div style="font-size:0.8125rem; color:var(--muted); margin-top:4px;">Typical: $2,000$6,000/year</div>
            </div>
            <button class="calc-btn" onclick="calcTrueHourly()">Calculate True Rate </button>
          </div>
        </div>

        <div id="thr-results" class="hidden" style="margin-top:24px;">
          <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap:16px; margin-bottom:24px;">
            <div class="card" style="text-align:center; padding:20px;">
              <div style="font-size:0.8125rem; font-weight:600; color:var(--muted); margin-bottom:8px; text-transform:uppercase; letter-spacing:0.05em;">Contract Rate</div>
              <div style="font-size:2rem; font-weight:800; color:var(--text);" id="thr-contract-rate"></div>
              <div style="font-size:0.8125rem; color:var(--muted);">per hour</div>
            </div>
            <div class="card" style="text-align:center; padding:20px; border-color:var(--accent);">
              <div style="font-size:0.8125rem; font-weight:600; color:var(--accent); margin-bottom:8px; text-transform:uppercase; letter-spacing:0.05em;">True Hourly Rate</div>
              <div style="font-size:2rem; font-weight:800; color:var(--accent);" id="thr-true-rate"></div>
              <div style="font-size:0.8125rem; color:var(--muted);">after all costs & time</div>
            </div>
            <div class="card" style="text-align:center; padding:20px;">
              <div style="font-size:0.8125rem; font-weight:600; color:var(--muted); margin-bottom:8px; text-transform:uppercase; letter-spacing:0.05em;">Rate Reduction</div>
              <div style="font-size:2rem; font-weight:800; color:var(--danger);" id="thr-reduction"></div>
              <div style="font-size:0.8125rem; color:var(--muted);">from stated rate</div>
            </div>
          </div>

          <div class="card" style="margin-bottom:24px;">
            <div class="card-title">RATE BREAKDOWN</div>
            <div class="row"><span class="row-label">Stated Contract Rate</span><span class="row-val" id="thr-b-contract"></span></div>
            <div class="row"><span class="row-label">SE Tax Impact (per hour)</span><span class="row-val" id="thr-b-setax" style="color:var(--danger);"></span></div>
            <div class="row"><span class="row-label">Health Insurance (per hour)</span><span class="row-val" id="thr-b-health" style="color:var(--danger);"></span></div>
            <div class="row"><span class="row-label">Malpractice (per hour)</span><span class="row-val" id="thr-b-malp" style="color:var(--danger);"></span></div>
            <div class="row"><span class="row-label">Benefits Gap (per hour)</span><span class="row-val" id="thr-b-benefits" style="color:var(--danger);"></span></div>
            <div class="row"><span class="row-label">Travel Expenses (per hour)</span><span class="row-val" id="thr-b-travel" style="color:var(--danger);"></span></div>
            <div class="row"><span class="row-label">Travel Time Cost (per hour)</span><span class="row-val" id="thr-b-traveltime" style="color:var(--danger);"></span></div>
            <div class="row" style="border-top:2px solid var(--accent); margin-top:8px; padding-top:8px;">
              <span class="row-label" style="font-weight:700;">True Hourly Rate</span>
              <span class="row-val" id="thr-b-true" style="color:var(--accent); font-weight:700;"></span>
            </div>
          </div>
          ...
          <div style="display:flex; gap:12px; flex-wrap:wrap;">
            <button class="calc-btn" onclick="shareResults('true-hourly')"> Share Results</button>
          </div>
        </div>
      </div>
      ...
      <div class="panel" id="panel-w2-1099">
        ...
        <div class="two-col">
          ...
          <div class="card">
            ...
            <button class="calc-btn" onclick="calcW21099()">Compare </button>
          </div>
        </div>
          ...
          <div class="two-col">
            <div class="compare-card" id="w2-card">
              ...
              <div class="row">
                ...
                <span class="row-val" id="w2-gross"></span>
              </div>
              <div class="row">
                <span class="row-label">Benefits</span>
                <span class="row-val" id="w2-ben"></span>
              </div>
              <div class="row">
                <span class="row-label">Taxes</span>
                <span class="row-val" id="w2-tax"></span>
              </div>
              <div class="row">
                <span class="row-label">Net</span>
                <span class="row-val" id="w2-net"></span>
              </div>
            </div>
            <div class="compare-card" id="c1099-card">
              ...
              <div class="row">
                ...
                <span class="row-val" id="c1099-gross"></span>
              </div>
              <div class="row">
                <span class="row-label">Expenses</span>
                <span class="row-val" id="c1099-expenses"></span>
              </div>
              <div class="row">
                <span class="row-label">Taxes</span>
                <span class="row-val" id="c1099-tax"></span>
              </div>
              <div class="row">
                <span class="row-label">Net</span>
                <span class="row-val" id="c1099-net"></span>
              </div>
              <div class="row">
                <span class="row-label">Diff vs W2</span>
                <span class="row-val" id="c1099-diff"></span>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- S-CORP EVALUATOR -->
      <div class="panel" id="panel-scorp">
        <div class="panel-header">
          <h2>S-Corp Evaluator</h2>
          <p>Should you form an S-Corporation? Compare Sole Prop/LLC vs S-Corp tax implications</p>
        </div>
        ...
        <div class="two-col">
          <div class="card">
            ...
            <div class="field">
              ...
              <input type="number" id="scorp-income" value="180000" min="0" step="1000">
            </div>
            <div class="field">
              <label>Annual Business Expenses</label>
              <input type="number" id="scorp-expenses" value="35000" min="0" step="1000">
            </div>
            <div class="field">
              <label>Filing Status</label>
              <select id="scorp-filing">
                <option value="single">Single</option>
                <option value="married" selected>Married Filing Jointly</option>
              </select>
            </div>
            <button class="calc-btn" onclick="calculateSCorp()">Calculate</button>
          </div>
          
          <div class="card">
            <div class="card-title">ABOUT S-CORP</div>
            <p style="font-size: 0.9375rem; color: var(--muted); line-height: 1.7; margin-bottom: 16px;">
              An S-Corporation can save you thousands in self-employment taxes by allowing you to split income between W2 salary (subject to SE tax) and distributions (not subject to SE tax).
            </p>
            <div style="background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.3); border-radius: 8px; padding: 16px; margin-bottom: 16px;">
              <div style="font-weight: 700; color: var(--accent); margin-bottom: 8px;"> Potential Savings</div>
              <div style="font-size: 0.875rem; color: var(--muted);">At $150k net income, you could save $5,000-8,000/year in taxes</div>
            </div>
            <div style="background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); border-radius: 8px; padding: 16px;">
              <div style="font-weight: 700; color: #ef4444; margin-bottom: 8px;"> Trade-offs</div>
              <div style="font-size: 0.875rem; color: var(--muted);">Payroll complexity, CPA costs (~$3k/year), reasonable salary requirements</div>
            </div>
          </div>
        </div>

        <div id="scorp-results" class="hidden">
          <div class="results-header">
            <h3>Sole Prop/LLC vs S-Corporation Comparison</h3>
          </div>

          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 32px;">
            <!-- Sole Prop Column -->
            <div style="background: var(--surface); border: 2px solid var(--border); border-radius: 12px; padding: 24px;">
              <h4 style="font-size: 1.25rem; font-weight: 700; margin-bottom: 20px; color: var(--text);">Sole Proprietorship / LLC</h4>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9375rem;">
                <span style="color: var(--muted);">Gross Income</span>
                <span style="font-weight: 700;" id="sp-gross"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9375rem;">
                <span style="color: var(--muted);">Business Expenses</span>
                <span style="font-weight: 700;" id="sp-expenses"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9375rem;">
                <span style="color: var(--muted);">Net Business Income</span>
                <span style="font-weight: 700;" id="sp-net"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9375rem; background: rgba(251, 191, 36, 0.1); margin: 0 -12px; padding: 10px 12px;">
                <span style="color: var(--muted);">Self-Employment Tax (15.3%)</span>
                <span style="font-weight: 700;" id="sp-se-tax"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9375rem;">
                <span style="color: var(--muted);">Federal Income Tax</span>
                <span style="font-weight: 700;" id="sp-income-tax"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9375rem;">
                <span style="color: var(--muted);">Retirement (Solo 401k)</span>
                <span style="font-weight: 700;" id="sp-retirement"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 16px 0 0; margin-top: 8px; border-top: 2px solid var(--accent); font-size: 1.125rem;">
                <span style="font-weight: 700;">Total Taxes</span>
                <span style="font-weight: 700;" id="sp-total-tax"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; font-size: 1.125rem;">
                <span style="font-weight: 700;">Take-Home Pay</span>
                <span style="font-weight: 700; color: var(--accent);" id="sp-take-home"></span>
              </div>
            </div>

            <!-- S-Corp Column -->
            <div id="scorp-col" style="background: var(--surface); border: 2px solid var(--border); border-radius: 12px; padding: 24px; position: relative;">
              <h4 style="font-size: 1.25rem; font-weight: 700; margin-bottom: 20px; color: var(--text);">S-Corporation</h4>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9375rem;">
                <span style="color: var(--muted);">Gross Income</span>
                <span style="font-weight: 700;" id="sc-gross"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9375rem;">
                <span style="color: var(--muted);">Business Expenses</span>
                <span style="font-weight: 700;" id="sc-expenses"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9375rem;">
                <span style="color: var(--muted);">Net Business Income</span>
                <span style="font-weight: 700;" id="sc-net"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9375rem; background: rgba(16, 185, 129, 0.1); margin: 0 -12px; padding: 10px 12px;">
                <span style="color: var(--muted);">Recommended W2 Salary</span>
                <span style="font-weight: 700;" id="sc-w2-salary"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9375rem; background: rgba(16, 185, 129, 0.1); margin: 0 -12px; padding: 10px 12px;">
                <span style="color: var(--muted);">Distributions (No SE Tax!)</span>
                <span style="font-weight: 700;" id="sc-distributions"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9375rem;">
                <span style="color: var(--muted);">Payroll Taxes (W2 only)</span>
                <span style="font-weight: 700;" id="sc-payroll-tax"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9375rem;">
                <span style="color: var(--muted);">Federal Income Tax</span>
                <span style="font-weight: 700;" id="sc-income-tax"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9375rem;">
                <span style="color: var(--muted);">S-Corp Admin Costs</span>
                <span style="font-weight: 700;" id="sc-admin"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid var(--border); font-size: 0.9375rem;">
                <span style="color: var(--muted);">Retirement (401k)</span>
                <span style="font-weight: 700;" id="sc-retirement"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 16px 0 0; margin-top: 8px; border-top: 2px solid var(--accent); font-size: 1.125rem;">
                <span style="font-weight: 700;">Total Taxes + Costs</span>
                <span style="font-weight: 700;" id="sc-total-tax"></span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 10px 0; font-size: 1.125rem;">
                <span style="font-weight: 700;">Take-Home Pay</span>
                <span style="font-weight: 700; color: var(--accent);" id="sc-take-home"></span>
              </div>
              <div id="sc-savings-row" style="display: flex; justify-content: space-between; padding: 12px; margin: 8px -12px 0; background: rgba(16, 185, 129, 0.15); border-radius: 8px; font-weight: 700; color: var(--accent);">
                <span>Annual Savings vs Sole Prop</span>
                <span id="sc-savings"></span>
              </div>
            </div>
          </div>

          <!-- Recommendation Box -->
          <div id="scorp-recommendation" style="background: linear-gradient(135deg, rgba(16, 185, 129, 0.1), rgba(59, 130, 246, 0.1)); border: 2px solid var(--accent); border-radius: 12px; padding: 24px; margin-bottom: 32px;">
            <h4 style="font-size: 1.25rem; font-weight: 700; margin-bottom: 12px; color: var(--text);">Recommendation</h4>
            <p id="scorp-rec-text" style="font-size: 1rem; line-height: 1.7; color: var(--text);"></p>
          </div>

          <!-- Reasonable Salary Benchmark -->
          <div id="scorp-salary-benchmark" style="margin-bottom: 32px; background: var(--surface); border: 1px solid var(--border); border-radius: 12px; padding: 24px;">
            <h4 style="font-size: 1.125rem; font-weight: 700; margin-bottom: 6px; color: var(--text);"> Reasonable Salary Benchmark</h4>
            <p style="font-size: 0.875rem; color: var(--muted); margin-bottom: 20px;">The IRS requires S-Corp owner-employees to pay themselves a salary comparable to what a non-owner would earn doing the same work. Use the BLS national median for your specialty as your starting point  your CPA can adjust based on hours worked, region, and experience.</p>
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 16px;">
              <div style="text-align: center; background: var(--surface2); border-radius: 8px; padding: 16px;">
                <div style="font-size: 0.75rem; color: var(--muted); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 6px;">Conservative Low</div>
                <div id="scorp-bench-low" style="font-size: 1.375rem; font-weight: 700; color: var(--warning);"></div>
              </div>
              <div style="text-align: center; background: rgba(16,185,129,0.08); border: 1px solid rgba(16,185,129,0.3); border-radius: 8px; padding: 16px;">
                <div style="font-size: 0.75rem; color: var(--accent); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 6px;">BLS Median (Safe Harbor)</div>
                <div id="scorp-bench-median" style="font-size: 1.375rem; font-weight: 700; color: var(--accent);"></div>
              </div>
              <div style="text-align: center; background: var(--surface2); border-radius: 8px; padding: 16px;">
                <div style="font-size: 0.75rem; color: var(--muted); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 6px;">High End</div>
                <div id="scorp-bench-high" style="font-size: 1.375rem; font-weight: 700; color: var(--text);"></div>
              </div>
            </div>
            <div id="scorp-bench-your-salary" style="display: flex; align-items: center; gap: 12px; padding: 12px 16px; background: var(--surface2); border-radius: 8px; margin-bottom: 12px;">
              <span style="font-size: 0.9375rem; color: var(--muted);">Your calculated W-2 salary:</span>
              <span id="scorp-bench-salary-val" style="font-size: 1rem; font-weight: 700; color: var(--text);"></span>
              <span id="scorp-bench-salary-status" style="font-size: 0.8125rem; font-weight: 600; padding: 3px 10px; border-radius: 4px;"></span>
            </div>
            <p id="scorp-bench-note" style="font-size: 0.8125rem; color: var(--muted); line-height: 1.6; font-style: italic;"></p>
            <p style="font-size: 0.75rem; color: var(--muted); margin-top: 8px;">Source: BLS May 2024 OES data (published April 2025). Ranges reflect national medians  consult a CPA for your specific situation.</p>
          </div>

          <!-- Section 127 Educational Assistance Benefit -->
          <div style="margin-bottom: 32px; background: linear-gradient(135deg, rgba(59,130,246,0.08), rgba(16,185,129,0.05)); border: 1px solid rgba(59,130,246,0.25); border-radius: 12px; padding: 24px;">
            <h4 style="font-size: 1.125rem; font-weight: 700; margin-bottom: 8px; color: var(--text);"> Section 127 Educational Assistance (S-Corp Benefit)</h4>
            <p style="font-size: 0.875rem; color: var(--muted); line-height: 1.7; margin-bottom: 16px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">
              <div style="background: rgba(16,185,129,0.1); border-radius: 6px; padding: 12px;">
                <div style="font-size: 0.8125rem; font-weight: 700; color: var(--accent); margin-bottom: 4px;"> What's Covered</div>
                <div style="font-size: 0.8125rem; color: var(--muted); line-height: 1.6;">Tuition, books, fees, equipment, and student loan repayments (now permanent per OBBBA 2025). Limit indexed for inflation after 2026.</div>
              </div>
              <div style="background: rgba(245,158,11,0.1); border-radius: 6px; padding: 12px;">
                <div style="font-size: 0.8125rem; font-weight: 700; color: var(--warning); margin-bottom: 4px;"> 2% Shareholder Restriction</div>
                <div style="font-size: 0.8125rem; color: var(--muted); line-height: 1.6;">Solo S-Corp owners (2%+ shareholders) cannot use this benefit for themselves unless â§5% of total costs go to such shareholders. Most beneficial if you have a W-2 employee (e.g., spouse).</div>
              </div>
            </div>
            <p style="font-size: 0.75rem; color: var(--muted);">Source: IRC ¬127 + One Big Beautiful Bill Act (OBBBA, Pub. L. No. 119-21, signed July 4, 2025)  made student loan repayment permanent. Always consult your CPA to set up a written, non-discriminatory plan.</p>
          </div>

          <!-- Mega Backdoor Roth -->
          <div style="margin-bottom: 32px; background: linear-gradient(135deg, rgba(139,92,246,0.08), rgba(16,185,129,0.05)); border: 1px solid rgba(139,92,246,0.25); border-radius: 12px; padding: 24px;">
            <h4 style="font-size: 1.125rem; font-weight: 700; margin-bottom: 8px; color: var(--text);"> Mega Backdoor Roth (S-Corp Solo 401k)</h4>
            <p style="font-size: 0.875rem; color: var(--muted); line-height: 1.7; margin-bottom: 16px;">
            ...
            <div style="background: var(--surface2); border-radius: 8px; padding: 16px; margin-bottom: 16px; font-size: 0.875rem;">
              <div style="font-weight: 700; color: var(--text); margin-bottom: 10px;">Example: S-Corp CRNA, age 40, $120k reasonable salary</div>
              <div style="display: flex; flex-direction: column; gap: 6px; color: var(--muted);">
                <div style="display: flex; justify-content: space-between;"><span>Employee contribution</span><span style="font-weight: 600; color: var(--text);">$23,500</span></div>
                <div style="display: flex; justify-content: space-between;"><span>Employer contribution (25% of $120k)</span><span style="font-weight: 600; color: var(--text);">$30,000</span></div>
                <div style="display: flex; justify-content: space-between; border-top: 1px solid var(--border); padding-top: 6px;"><span>Direct contributions total</span><span style="font-weight: 600; color: var(--text);">$53,500</span></div>
                <div style="display: flex; justify-content: space-between;"><span>Gap to $70k limit</span><span style="font-weight: 700; color: #8b5cf6;">$16,500  Mega Backdoor Roth</span></div>
              </div>
            </div>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">
              <div style="background: rgba(16,185,129,0.1); border-radius: 6px; padding: 12px;">
                <div style="font-size: 0.8125rem; font-weight: 700; color: var(--accent); margin-bottom: 4px;">How it works</div>
                <div style="font-size: 0.8125rem; color: var(--muted); line-height: 1.6;">Contribute the gap amount as "voluntary after-tax" (VAT) to your solo 401k, then immediately convert it to the Roth subaccount. All future growth is tax-free.</div>
              </div>
              <div style="background: rgba(245,158,11,0.1); border-radius: 6px; padding: 12px;">
                <div style="font-size: 0.8125rem; font-weight: 700; color: var(--warning); margin-bottom: 4px;"> Requires a custom plan</div>
                <div style="font-size: 0.8125rem; color: var(--muted); line-height: 1.6;">Free plans at Fidelity, Schwab, and Vanguard do NOT support VAT contributions. You need a paid custom solo 401k (MySolo401k.net, Rocket Dollar, Ubiquity  ~$100$300/yr). Offset by the $1,500 EACA tax credit (Form 8881).</div>
              </div>
            </div>
            <p style="font-size: 0.75rem; color: var(--muted);">Source: IRS ¬415(c), SECURE 2.0 Act. S-Corp solo 401k has 3 subaccounts: pretax, Roth, and voluntary after-tax (VAT). Consult your CPA before setting up.</p>
          </div>

          <!-- How We Calculated -->
          <div style="margin-top: 40px;">
            <h4 style="font-size: 1.25rem; font-weight: 700; margin-bottom: 24px; color: var(--text);">How We Calculated This</h4>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px;">
              <div style="background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 20px;">
                <h5 style="font-size: 1rem; font-weight: 700; margin-bottom: 12px;">Self-Employment Tax</h5>
                <p style="font-size: 0.875rem; color: var(--muted); line-height: 1.6;">For Sole Prop: 15.3% on 92.35% of net income. You pay both employer + employee portions.</p>
                <div style="background: rgba(16, 185, 129, 0.1); padding: 8px 12px; border-radius: 4px; font-family: monospace; font-size: 0.8125rem; margin-top: 12px;">
                  Net √ó 92.35% √ó 15.3%
                </div>
              </div>
              <div style="background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 20px;">
                <h5 style="font-size: 1rem; font-weight: 700; margin-bottom: 12px;">S-Corp Strategy (BLS Safe Harbor)</h5>
                <p style="font-size: 0.875rem; color: var(--muted); line-height: 1.6;">We use your profession's BLS median salary as your W2 salary. This "safe harbor" approach is defensible to the IRS and maximizes distributions (which save 15.3% SE tax).</p>
                <div style="background: rgba(16, 185, 129, 0.1); padding: 8px 12px; border-radius: 4px; font-family: monospace; font-size: 0.8125rem; margin-top: 12px;">
                  W2 = BLS Median (e.g. $214k for CRNA)
                </div>
              </div>
              <div style="background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 20px;">
                <h5 style="font-size: 1rem; font-weight: 700; margin-bottom: 12px;">Distributions</h5>
                <p style="font-size: 0.875rem; color: var(--muted); line-height: 1.6;">Remaining profit is distributed. NOT subject to SE tax (15.3% savings!).</p>
                <div style="background: rgba(16, 185, 129, 0.1); padding: 8px 12px; border-radius: 4px; font-family: monospace; font-size: 0.8125rem; margin-top: 12px;">
                  Distributions = Net - Salary
                </div>
              </div>
              <div style="background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 20px;">
                <h5 style="font-size: 1rem; font-weight: 700; margin-bottom: 12px;">Retirement</h5>
                <p style="font-size: 0.875rem; color: var(--muted); line-height: 1.6;"><strong>Sole Prop:</strong> 20% of net + $23k<br><strong>S-Corp:</strong> 25% of W2 + $23k</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="panel" id="panel-tax">
        ...
        <div class="two-col">
          ...
          <div class="card">
            ...
            <button class="calc-btn" onclick="calcTax()">Estimate </button>
          </div>
        </div>
        <div id="tx-results" class="hidden">
          <div class="results-grid">
            <div class="stat-box highlight">
              ...
              <div class="stat-value" id="tx-rate"></div>
            </div>
            <div class="stat-box">
              <div class="stat-label">SE TAX</div>
              <div class="stat-value danger" id="tx-se"></div>
            </div>
            <div class="stat-box">
              <div class="stat-label">TOTAL DEDUCTIONS</div>
              <div class="stat-value success" id="tx-ded"></div>
            </div>
            <div class="stat-box">
              <div class="stat-label">QUARTERLY PAYMENT</div>
              <div class="stat-value neutral" id="tx-q"></div>
            </div>
          </div>
          <div class="insight">
      ...
      <div class="panel" id="panel-home">
        ...
        <div class="two-col">
          ...
          <div class="card">
            ...
            <button class="calc-btn" onclick="calcHome()">Calculate </button>
          </div>
        </div>
        <div id="hm-results" class="hidden">
          <div class="results-grid">
            <div class="stat-box highlight">
              ...
              <div class="stat-value" id="hm-months"></div>
              <div class="stat-sub" id="hm-date"></div>
            </div>
            <div class="stat-box">
              <div class="stat-label">DOWN PAYMENT</div>
              <div class="stat-value neutral" id="hm-down-amt"></div>
            </div>
            <div class="stat-box">
              <div class="stat-label">MONTHLY PAYMENT</div>
              <div class="stat-value neutral" id="hm-payment"></div>
            </div>
            <div class="stat-box">
              <div class="stat-label">DTI RATIO</div>
              <div class="stat-value" id="hm-dti"></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="panel" id="panel-loan">
        ...
        <div class="two-col">
          ...
          <div class="card">
            ...
            <button class="calc-btn" onclick="calcLoan()">Compare </button>
          </div>
        </div>
          ...
          <div class="two-col">
            <div class="compare-card">
              ...
              <div class="row">
                ...
                <span class="row-val" id="ln-std-int"></span>
              </div>
              <div class="row">
                <span class="row-label">Payoff Time</span>
                <span class="row-val" id="ln-std-time"></span>
              </div>
              <div class="row">
                <span class="row-label">Total Paid</span>
                <span class="row-val" id="ln-std-total"></span>
              </div>
            </div>
            <div class="compare-card winner">
              ...
              <div class="row">
                <span class="row-label">Total Interest</span>
                <span class="row-val" id="ln-agg-int"></span>
              </div>
              <div class="row">
                <span class="row-label">Payoff Time</span>
                <span class="row-val" id="ln-agg-time"></span>
              </div>
              <div class="row">
                <span class="row-label">Total Paid</span>
                <span class="row-val" id="ln-agg-total"></span>
              </div>
              <div class="row">
                <span class="row-label">Savings</span>
                <span class="row-val" id="ln-save"></span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="panel" id="panel-income">
        ...
        <div class="two-col">
          ...
          <div class="card">
            ...
            <button class="calc-btn" onclick="calcIncome()">Project </button>
          </div>
        </div>
        <div id="it-results" class="hidden">
          <div class="results-grid">
            <div class="stat-box highlight">
              ...
              <div class="stat-value" id="it-y5"></div>
            </div>
            <div class="stat-box">
              <div class="stat-label">YEAR 10 INCOME</div>
              <div class="stat-value neutral" id="it-y10"></div>
            </div>
            <div class="stat-box">
              <div class="stat-label">TOTAL GROWTH</div>
              <div class="stat-value success" id="it-growth"></div>
            </div>
            <div class="stat-box">
              <div class="stat-label">CURRENT TOTAL</div>
              <div class="stat-value neutral" id="it-cur"></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="panel" id="panel-benchmarks">
        <div class="panel-header">
          <h2>Healthcare Provider Benchmarks</h2>
          <p>Industry-verified expense, retirement, and tax data for 2025</p>
        </div>

        <!-- PROFESSION SELECTOR -->
        <div class="card" style="margin-bottom: 24px;">
          <div class="card-title">YOUR PROFESSION</div>
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
            <div class="field">
              <label>Profession</label>
              <select id="bench-profession" onchange="renderBenchmarks()">
                <option value="crna">CRNA</option>
                <option value="md_hospitalist">Physician  Hospitalist</option>
                <option value="md_em">Physician  Emergency Medicine</option>
                <option value="md_anesthesia">Physician  Anesthesiology</option>
            ...
            <div class="field">
              <label>Annual 1099 Income</label>
              <input type="number" id="bench-income" value="250000" min="0" step="10000" oninput="renderBenchmarks()">
            </div>
          </div>
        </div>

        <!-- SECTION 1: SELF-EMPLOYMENT EXPENSES -->
        <div style="margin-bottom: 32px;">
          <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
            <div style="width: 36px; height: 36px; background: linear-gradient(135deg, #ef4444, #dc2626); border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 1.125rem;"></div>
            <div>
              <div style="font-size: 1.125rem; font-weight: 700; color: var(--text);">Self-Employment Expense Benchmarks</div>
              <div style="font-size: 0.8125rem; color: var(--muted);">Typical annual costs for independent healthcare providers</div>
            </div>
          </div>
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px;" id="bench-expenses-grid">
            <!-- Populated by JS -->
          </div>
          <div style="margin-top: 12px; padding: 12px 16px; background: rgba(239, 68, 68, 0.06); border: 1px solid rgba(239, 68, 68, 0.2); border-radius: 8px; font-size: 0.8125rem; color: var(--muted);">
        ...
        <div style="margin-bottom: 32px;">
          <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
            <div style="width: 36px; height: 36px; background: linear-gradient(135deg, #8b5cf6, #7c3aed); border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 1.125rem;">¶</div>
            <div>
              <div style="font-size: 1.125rem; font-weight: 700; color: var(--text);">Retirement Contribution Benchmarks</div>
              <div style="font-size: 0.8125rem; color: var(--muted);">2025 IRS limits  Solo 401(k) vs SEP-IRA for your income level</div>
            </div>
          </div>
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;" id="bench-retirement-grid">
            <!-- Populated by JS -->
          </div>
          <div style="margin-top: 12px; padding: 12px 16px; background: rgba(139, 92, 246, 0.06); border: 1px solid rgba(139, 92, 246, 0.2); border-radius: 8px; font-size: 0.8125rem; color: var(--muted);">
        ...
        <!-- SECTION 3: TAX BURDEN COMPARISON -->
        <div style="margin-bottom: 32px;" id="bench-tax-section">
          <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
            <div style="width: 36px; height: 36px; background: linear-gradient(135deg, #10b981, #059669); border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 1.125rem;"></div>
            <div style="flex: 1;">
              <div style="display: flex; align-items: center; gap: 10px;">
                <div style="font-size: 1.125rem; font-weight: 700; color: var(--text);">Tax Burden Comparison</div>
                <span style="background: var(--accent); color: white; font-size: 0.625rem; padding: 2px 7px; border-radius: 4px; font-weight: 700; letter-spacing: 0.03em;">PRO</span>
              </div>
              <div style="font-size: 0.8125rem; color: var(--muted);">Estimated effective rates at your income across structures (married filing jointly, no state tax)</div>
            </div>
          </div>

          <!-- PRO CONTENT (shown when isPro) -->
          <div id="bench-tax-pro-content" class="hidden">
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin-bottom: 16px;" id="bench-tax-grid">
              <!-- Populated by JS -->
            </div>
              ...
              <table style="width: 100%; border-collapse: collapse; font-size: 0.875rem;">
                <thead>
                  <tr style="background: var(--surface2);">
                    <th style="text-align: left; padding: 10px 14px; color: var(--muted); font-weight: 600; border-bottom: 1px solid var(--border);">Tax Category</th>
                    <th style="text-align: right; padding: 10px 14px; color: var(--muted); font-weight: 600; border-bottom: 1px solid var(--border);">W-2 Employee</th>
                    <th style="text-align: right; padding: 10px 14px; color: var(--muted); font-weight: 600; border-bottom: 1px solid var(--border);">1099 Sole Prop</th>
                    <th style="text-align: right; padding: 10px 14px; color: var(--accent); font-weight: 600; border-bottom: 1px solid var(--border);">S-Corp Optimized</th>
                  </tr>
                </thead>
                <tbody id="bench-tax-table-body">
                  <!-- Populated by JS -->
                </tbody>
              </table>
            </div>
            <div style="margin-top: 12px; padding: 12px 16px; background: rgba(16, 185, 129, 0.06); border: 1px solid rgba(16, 185, 129, 0.2); border-radius: 8px; font-size: 0.8125rem; color: var(--muted);">
          ...
          <!-- PAYWALL (shown when !isPro) -->
          <div id="bench-tax-paywall" class="hidden" style="border: 1px solid var(--border); border-radius: 12px; padding: 32px; text-align: center; background: var(--surface);">
            <div style="font-size: 2rem; margin-bottom: 12px;"></div>
            <div style="font-size: 1rem; font-weight: 700; color: var(--text); margin-bottom: 8px;">Your personalized tax breakdown is a Pro feature</div>
            <div style="font-size: 0.875rem; color: var(--muted); margin-bottom: 20px; max-width: 420px; margin-left: auto; margin-right: auto; line-height: 1.6;">
            ...
            <button class="calc-btn" onclick="showUpgradeModal()" style="margin: 0 auto; display: inline-flex; padding: 10px 28px; font-size: 0.9375rem;">Unlock with Pro</button>
          </div>
        </div>
        ...
        <div>
          <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
            <div style="width: 36px; height: 36px; background: linear-gradient(135deg, #3b82f6, #2563eb); border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 1.125rem;"></div>
            <div>
              <div style="font-size: 1.125rem; font-weight: 700; color: var(--text);">Quick Reference: All Professions</div>
              <div style="font-size: 0.8125rem; color: var(--muted);">BLS May 2024 salaries ¬∑ typical malpractice ¬∑ CME allowances</div>
            </div>
          </div>
          <div style="overflow-x: auto;">
            <table style="width: 100%; border-collapse: collapse; font-size: 0.8125rem;">
              <thead>
                <tr style="background: var(--surface2);">
                  <th style="text-align: left; padding: 10px 14px; color: var(--muted); font-weight: 600; border-bottom: 1px solid var(--border);">Profession</th>
                  <th style="text-align: right; padding: 10px 14px; color: var(--muted); font-weight: 600; border-bottom: 1px solid var(--border);">BLS Median Salary</th>
                  <th style="text-align: right; padding: 10px 14px; color: var(--muted); font-weight: 600; border-bottom: 1px solid var(--border);">Malpractice Range</th>
                  <th style="text-align: right; padding: 10px 14px; color: var(--muted); font-weight: 600; border-bottom: 1px solid var(--border);">Avg CME Allowance</th>
                  <th style="text-align: right; padding: 10px 14px; color: var(--muted); font-weight: 600; border-bottom: 1px solid var(--border);">S-Corp Threshold</th>
                </tr>
              </thead>
              <tbody>
                <tr style="border-bottom: 1px solid var(--border);">
                  <td style="padding: 10px 14px; font-weight: 600; color: var(--text);">CRNA</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--text);">$214,000</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$6,000$15,000</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$3,800$4,073</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--accent);">&gt;$150k net</td>
                </tr>
                <tr style="border-bottom: 1px solid var(--border); background: var(--surface2);">
                  <td style="padding: 10px 14px; font-weight: 600; color: var(--text);">Physician  Hospitalist</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--text);">~$290,000</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$8,000$20,000</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$3,800$4,073</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--accent);">&gt;$150k net</td>
                </tr>
                <tr style="border-bottom: 1px solid var(--border);">
                  <td style="padding: 10px 14px; font-weight: 600; color: var(--text);">Physician  EM</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--text);">~$345,000</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$20,000$40,000</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$3,800$4,073</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--accent);">&gt;$150k net</td>
                </tr>
                <tr style="border-bottom: 1px solid var(--border); background: var(--surface2);">
                  <td style="padding: 10px 14px; font-weight: 600; color: var(--text);">Physician  Anesthesia</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--text);">~$430,000</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$15,000$30,000</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$3,800$4,073</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--accent);">&gt;$150k net</td>
                </tr>
                <tr style="border-bottom: 1px solid var(--border);">
                  <td style="padding: 10px 14px; font-weight: 600; color: var(--text);">Physician  Family Practice</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--text);">~$245,000</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$5,000$12,000</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$3,800$4,073</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--accent);">&gt;$150k net</td>
                </tr>
                <tr style="border-bottom: 1px solid var(--border); background: var(--surface2);">
                  <td style="padding: 10px 14px; font-weight: 600; color: var(--text);">Nurse Practitioner</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--text);">$129,210</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$1,200$4,500</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$2,083$2,862</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--accent);">&gt;$80k net</td>
                </tr>
                <tr style="border-bottom: 1px solid var(--border);">
                  <td style="padding: 10px 14px; font-weight: 600; color: var(--text);">Physician Assistant</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--text);">$130,020</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$1,700$2,650</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$2,083$2,862</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--accent);">&gt;$80k net</td>
                </tr>
                <tr>
                  <td style="padding: 10px 14px; font-weight: 600; color: var(--text);">Registered Nurse</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--text);">$93,600</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$500$2,000</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--muted);">$2,083$2,862</td>
                  <td style="text-align: right; padding: 10px 14px; color: var(--accent);">&gt;$80k net</td>
                </tr>
              </tbody>
...
<div class="modal-overlay" id="upgradeModal">
  <div class="modal">
    ...
    <p style="margin-bottom: 20px;">Unlock the full LocumsLab toolkit  pay once, use forever.</p>
    <div class="pricing-box">
      <div class="pricing-amount">$39 <span style="font-size: 1rem; font-weight: 400; color: var(--muted);">one-time</span></div>
      <div class="pricing-features">
         ...
         Compare up to 5 scenarios side-by-side<br>
         Export professional PDF reports<br>
         Lifetime access  use forever
      </div>
      <div style="background: rgba(16, 185, 129, 0.08); border: 1px solid rgba(16, 185, 129, 0.3); border-radius: 8px; padding: 12px; margin-top: 16px; font-size: 0.875rem; color: var(--muted);">
    ...
    <div class="modal-actions">
      ...
      <button class="modal-btn primary" id="checkoutBtn" onclick="handleUpgrade()">Upgrade for $39 </button>
    </div>
  </div>
</div>

<!-- S-CORP PAYWALL MODAL -->
<div class="modal-overlay" id="scorpPaywallModal">
  <div class="modal">
    <h3> S-Corp Evaluator (Pro Feature)</h3>
    <p style="margin-bottom: 20px;">Discover if forming an S-Corporation could save you $5,000-15,000/year in self-employment taxes.</p>
    <div style="background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 16px; margin-bottom: 20px;">
      <div style="font-weight: 700; margin-bottom: 12px; color: var(--text);">What You'll Get:</div>
      <div style="font-size: 0.9375rem; color: var(--muted); line-height: 1.7;">
    ...
    <div style="background: rgba(59, 130, 246, 0.1); border: 1px solid rgba(59, 130, 246, 0.3); border-radius: 8px; padding: 12px; margin-bottom: 20px; font-size: 0.875rem;">
       Unlock the S-Corp Evaluator and all Pro features for a one-time payment of $39.
    </div>
    <div class="modal-actions">
      <button class="modal-btn secondary" onclick="hideScorpPaywall()">Maybe Later</button>
      <button class="modal-btn primary" onclick="hideScorpPaywall(); showUpgradeModal();">Upgrade for $39 </button>
    </div>
  </div>
</div>

<!-- COMPARE PAYWALL MODAL -->
<div class="modal-overlay" id="comparePaywallModal">
  <div class="modal">
    <h3> Compare Scenarios (Pro Feature)</h3>
    <p style="margin-bottom: 20px;">See up to 5 saved scenarios side-by-side with recommendations.</p>
    <div style="background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 16px; margin-bottom: 20px;">
      <div style="font-weight: 700; margin-bottom: 12px; color: var(--text);">Compare:</div>
      <div style="font-size: 0.9375rem; color: var(--muted); line-height: 1.7;">
    ...
    <div class="modal-actions">
      <button class="modal-btn secondary" onclick="hideComparePaywall()">Maybe Later</button>
      <button class="modal-btn primary" onclick="handleUpgrade()">Unlock Compare  $39</button>
    </div>
  </div>
</div>
  ...
  <div class="modal" style="max-width: 700px;">
    ...
    <div style="max-height: 60vh; overflow-y: auto; margin: 20px 0;">
      ...
      <div style="margin-bottom: 24px;">
        <h4 style="font-size: 1rem; font-weight: 700; margin-bottom: 8px;">How accurate are the calculations?</h4>
      ...
      <div style="margin-bottom: 24px;">
        <h4 style="font-size: 1rem; font-weight: 700; margin-bottom: 8px;">What's included in the S-Corp Evaluator?</h4>
        <p style="font-size: 0.9375rem; color: var(--muted); line-height: 1.6;">
      ...
      <div style="margin-bottom: 24px;">
        <h4 style="font-size: 1rem; font-weight: 700; margin-bottom: 8px;">Can I export my scenarios?</h4>
        <p style="font-size: 0.9375rem; color: var(--muted); line-height: 1.6;">
      ...
      <div style="margin-bottom: 24px;">
        <h4 style="font-size: 1rem; font-weight: 700; margin-bottom: 8px;">Is this only for CRNAs?</h4>
        <p style="font-size: 0.9375rem; color: var(--muted); line-height: 1.6;">
      ...
      <div style="margin-bottom: 24px;">
        <h4 style="font-size: 1rem; font-weight: 700; margin-bottom: 8px;">Where is my data stored?</h4>
        <p style="font-size: 0.9375rem; color: var(--muted); line-height: 1.6;">
          All data is stored locally in your browser (localStorage). We don't transmit your scenario data to any server. This means your data is private, but clearing your browser data will remove your saved scenarios.
        </p>
      </div>
      
      <div style="margin-bottom: 24px;">
        <h4 style="font-size: 1rem; font-weight: 700; margin-bottom: 8px;">What's the difference between Free and Pro?</h4>
        <p style="font-size: 0.9375rem; color: var(--muted); line-height: 1.6;">
          <strong>Free:</strong> All calculators, save 1 scenario<br>
          <strong>Pro ($39 one-time):</strong> S-Corp Evaluator, Benchmarks, unlimited scenarios, comparison view, PDF export, lifetime access
        </p>
      </div>
      
      <div style="margin-bottom: 24px;">
        <h4 style="font-size: 1rem; font-weight: 700; margin-bottom: 8px;">Is Pro a subscription?</h4>
        <p style="font-size: 0.9375rem; color: var(--muted); line-height: 1.6;">
          No! Pro is a <strong>one-time payment</strong> of $39. Pay once, use forever. No recurring charges.
        </p>
      </div>
      
    </div>
    <div class="modal-actions">
      <button class="modal-btn primary" onclick="hideFAQ()">Close</button>
    </div>
  </div>
</div>
...
<div class="modal-overlay" id="contactModal">
  <div class="modal" style="max-width: 500px;">
    <h3>Contact Us</h3>
    <p style="margin: 16px 0; font-size: 0.9375rem; color: var(--muted); line-height: 1.7;">
      Have questions, feedback, or need help? We typically respond within 24 hours.
    </p>
    <div id="contactFormWrap">
      <div style="margin-bottom: 14px;">
        <label style="display:block; font-size:0.875rem; font-weight:600; margin-bottom:6px; color:var(--text);">Your Email</label>
        <input type="email" id="contactEmail" placeholder="you@email.com"
          style="width:100%; background:var(--surface); border:1px solid var(--border); border-radius:8px; padding:10px 14px; color:var(--text); font-size:0.9375rem; font-family:inherit; outline:none;">
      </div>
      <div style="margin-bottom:14px;">
        <label style="display:block; font-size:0.875rem; font-weight:600; margin-bottom:6px; color:var(--text);">Subject</label>
      ...
      <div style="margin-bottom:20px;">
        <label style="display:block; font-size:0.875rem; font-weight:600; margin-bottom:6px; color:var(--text);">Message</label>
        <textarea id="contactMessage" rows="4" placeholder="Your message..."
          style="width:100%; background:var(--surface); border:1px solid var(--border); border-radius:8px; padding:10px 14px; color:var(--text); font-size:0.9375rem; font-family:inherit; outline:none; resize:vertical;"></textarea>
      </div>
      <div id="contactError" style="color:#ef4444; font-size:0.875rem; margin-bottom:12px; display:none;"></div>
      <div class="modal-actions">
        <button class="modal-btn secondary" onclick="hideContact()">Cancel</button>
        <button class="modal-btn primary" id="contactSubmitBtn" onclick="submitContact()">Send Message</button>
      </div>
    </div>
    <div id="contactSuccess" style="display:none; text-align:center; padding:24px 0;">
      <div style="font-size:2rem; margin-bottom:12px;"></div>
      <div style="font-weight:700; color:var(--text); margin-bottom:8px;">Message sent!</div>
      <div style="color:var(--muted); font-size:0.9375rem; margin-bottom:24px;">We'll get back to you within 24 hours.</div>
      <button class="modal-btn primary" onclick="hideContact()">Close</button>
    </div>
  </div>
</div>
...
<div class="modal-overlay" id="shareModal">
  <div class="modal" style="max-width: 520px;">
    ...
    <p style="color:var(--muted); font-size:0.9375rem; margin:12px 0 20px;">Download the image card to post on Reddit, Facebook, or anywhere  or copy the link to share directly.</p>

    <!-- IMAGE CARD PREVIEW -->
    <div style="margin-bottom:20px; border-radius:10px; overflow:hidden; border:1px solid var(--border);">
      <canvas id="shareCanvas" style="width:100%; display:block;"></canvas>
    </div>

    <!-- ACTIONS ROW -->
    <div style="display:flex; gap:10px; margin-bottom:20px; flex-wrap:wrap;">
      <button class="calc-btn" onclick="downloadShareCard()" style="flex:1; display:flex; align-items:center; justify-content:center; gap:8px; padding:12px 16px;">
        ¨á Download Image
      </button>
      <button class="calc-btn" onclick="copyShareText()" id="copyTextBtn" style="flex:1; background:var(--surface2); color:var(--text); border:1px solid var(--border); display:flex; align-items:center; justify-content:center; gap:8px; padding:12px 16px;">
         Copy Post Text
      </button>
    </div>

    <!-- LINK ROW -->
    <div style="font-size:0.8125rem; color:var(--muted); margin-bottom:8px; font-weight:600;">Or share a link:</div>
    <div style="display:flex; gap:8px; margin-bottom:20px;">
      <input type="text" id="shareUrlInput" readonly
        style="flex:1; background:var(--surface); border:1px solid var(--border); border-radius:8px; padding:10px 14px; color:var(--text); font-size:0.875rem; font-family:inherit; outline:none;">
      <button class="calc-btn" id="copyShareBtn" onclick="copyShareUrl()" style="padding:10px 18px; white-space:nowrap;">Copy Link</button>
    </div>
    <div id="shareModalLoading" style="text-align:center; padding:12px; color:var(--muted); font-size:0.875rem; display:none;">Generating link...</div>
    <div class="modal-actions">
      <button class="modal-btn primary" onclick="hideShareModal()">Done</button>
    </div>
  </div>
</div>
...
<div class="modal-overlay" id="legalModal">
  <div class="modal" style="max-width: 700px;">
    ...
    <div id="legalBody" style="max-height: 60vh; overflow-y: auto; margin: 20px 0; font-size: 0.9375rem; line-height: 1.7;"></div>
    <div class="modal-actions">
      <button class="modal-btn primary" onclick="hideLegalModal()">Close</button>
    </div>
  </div>
</div>
...
<footer>
  <p><strong>LocumsLab</strong>  Compare contracts. Model take-home. Decide with clarity.</p>
  <p style="margin-top:8px; font-size:0.75rem; color: var(--muted); max-width: 900px;">
    ...
    <span style="display:inline-block; margin-left:10px;">Scenarios are saved securely to your account.</span>
  </p>
  <p style="margin-top:10px; font-size:0.75rem; color: var(--muted);">
    <a href="#" onclick="showFAQ(); return false;" style="color: var(--muted); text-decoration: underline;">FAQ</a>
    &nbsp;¬∑&nbsp;
    ...
    &nbsp;¬∑&nbsp;
    <a href="#" onclick="showContact(); return false;" style="color: var(--muted); text-decoration: underline;">Contact</a>
  </p>
</footer>
...
const BENCHMARKS = {
  ...
  md_hospitalist: {
    name: 'Hospitalist',
    staff: {
      // BLS May 2024: Physicians & Surgeons median â•$239,200. MGMA hospitalist ~$285k.
    ...
    // White Coat Investor: EM 10th90th percentile $205k$384k; hospitalists lower.
...
function calculateSCorp() {
  ...
  //          29-1141 (RN $93,600), physicians â•$239,200 + MGMA specialty breakdowns
  const BLS_SALARIES = {
    'crna': 214000,
...
Note: Always work with a CPA to ensure proper setup and compliance.`;
  ...
  if (benchData && benchData.scorp_salary) {
    ...
    if (reasonableSalary < salaryBench.low) {
      ...
      statusEl.textContent = 'π Above median  conservative';
    ...
    return `
      <div class="scenario-card" onclick="loadScenario('${s.id}')">
        <div class="scenario-header">
          ...
          <button class="scenario-action-btn" onclick="event.stopPropagation(); deleteScenario('${s.id}')">ó</button>
        </div>
...
function calcQuickStart() {
  ...
  document.getElementById('qs-next-steps').innerHTML = `
    ...
    4. <strong>Note:</strong> Your W-2 PTO (${ptoDays} days âà ${fmt(ptoValue)}/yr value) disappears as 1099  factor in unpaid time between contracts.
  `;
...
function drawShareCard(calculator, data) {
  ...
  // Card dimensions: 1200√ó630 (standard OG image ratio)
  const W = 1200, H = 630;
  canvas.width  = W * dpr;
  canvas.height = H * dpr;
...
function renderBenchmarks() {
  ...
  const expItems = [
    { label: 'Malpractice Insurance', icon: 'õ', low: data.malpractice.low, mid: data.malpractice.mid, high: data.malpractice.high, color: '#ef4444' },
    ...
    { label: 'CPA / Payroll', icon: 'æ', low: data.cpa.low, mid: data.cpa.mid, high: data.cpa.high, color: '#10b981' },
  ];
  ...
  // Approximation: employer contribution âà 20% of net self-employment income (Schedule C)
  ...
  // SEP-IRA: 25% of compensation (âà20% of net SE income), max $70k
    ...
    <div style="background: var(--surface); border: 1px solid var(--border); border-radius: 12px; padding: 24px;">
      ...
      <div style="display: flex; flex-direction: column; gap: 8px; font-size: 0.8125rem;">
        ...
        <div style="display: flex; justify-content: space-between; padding: 8px 12px; background: var(--surface2); border-radius: 6px;">
          ...
          <span style="font-weight: 700; color: #ef4444;">ó None</span>
        </div>
        <div style="display: flex; justify-content: space-between; padding: 8px 12px; background: var(--surface2); border-radius: 6px;">
          <span style="color: var(--muted);">Backdoor Roth compatible</span>
  ...
  ">
    ...
    <div style="flex:1; overflow-y:auto; padding:24px;" id="guide-drawer-body">
      ...
      ${[
        ...
        { n:'03', title:'LLC & S-Corp Explained', content:'When to form each. S-Corp distributions save you 15.3% SE tax on income above your "reasonable salary." At $250k net income with a $120k salary, that\'s roughly $19,000/year saved. Admin costs ~$3,500/yr (CPA + payroll service). Typically worth it above $80100k net income.' },
        ...
        { n:'05', title:'Malpractice Insurance', content:'Claims-made vs occurrence policies. Claims-made is cheaper annually but requires "tail coverage" (~1.52√ó annual premium) when you leave. CRNA individual policies: $6,000$15,000/yr. Always verify your contract specifies who provides coverage and whether tail is included.' },
        ...
        { n:'09', title:'Payroll & Bookkeeping', content:'Gusto for payroll (~$500/yr) handles W-2 for S-Corp owners, 940/941 filings, and year-end forms. QuickBooks Self-Employed or Keeper for bookkeeping. Set aside 2530% of each payment for taxes in a separate account. Bill from your LLC/S-Corp, not personally.' },
